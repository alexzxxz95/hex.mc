...
Модулі (modules/*.py)
-Це повноцінні режими застосунку, які додаються в бічне меню та мають власну панель налаштувань.

  - Складність: Вища, оскільки розробка вимагає повної реалізації контракту HexModuleBase.
  - Ядро та автореєстрація: Зміни в коді ядра не потрібні — модуль автореєструється системою.
  - Розповсюдження: Файл додається у папку modules/ безпосередньо під час встановлення застосунку.
  - Документація: [MODULES_DEV_GUIDE.md](MODULES_DEV_GUIDE.md)

Скрипти (SCRIPTS-режим)
-Це окремі .py-файли, які виконуються та завантажуються безпосередньо всередині вже існуючого режиму SCRIPTS.

  - Складність: Нижча — для роботи достатньо написати дві функції: setup_ui() та update().
  - Ядро та автореєстрація: Зміни в коді ядра не потрібні. Файл можна покласти в будь-яку зручну папку та вибрати через інтерфейс застосунку.
  - Розповсюдження: Підвантажується користувачем «на льоту» в будь-який момент, без необхідності перевстановлювати застосунок.
  - Документація: [SCRIPTS_DEV_GUIDE.md](SCRIPTS_DEV_GUIDE.md)
